...
describe('GET /autor', ()=>{
    test('esperamos un 200 al hacer el get', async () =>{
        const result = await request(app).get('/autor').send();
        expect(result.status).toBe(200);
    })

    test('espera 200 si se encontro con id', async () =>{
        const  id = 1;
        const result = await request(app).get(`/autor/${id}`).send();
        expect(result.status).toBe(200)
    })
    ...
    test('espera un 404 si al buscar por id no se encuentra el usuario', async ()=>{
        const id = 999;
        const res = await request(app).get(`/autor${id}`).send();
...
describe('POST /autor', () =>{
   ...
   let  personasPrueba = {name : 'joaco', birthdate : '2025-03-03'};
    ...
    test('espera 201 si se creo el usuario', async ()=>{
        const result = await request(app).post('/autor').send(personasPrueba)
        expect(result.status).toBe(201);
    })

    test('espera que el  content-type: application/json en el encabezado', async () =>{
        const res = await request(app).post('/autor').send(personasPrueba);
    ...
    test('espera un 400 si no se mando un dato', async () =>{
        const res = await request(app).post('/autor').send({birthdate : "2006-09-02"});
        expect(res.status).toBe(400);
    })

    test('espera 400 si la fecha no esta en su formato', async () =>{
        const res = await request(app).post('/autor').send({name : 'pepe', birthdate : 'abril'});
        expect(res.status).toBe(400);
    })
